...
import { METRIC_LABELS, isStandingMetric } from "@/lib/quota-labels";

// The vocabulary lives in `lib/quota-labels.ts`, which imports no database —
// client components need the labels, and importing them from here pulled `pg`
// into the browser bundle. Re-exported so server code keeps one import.
export * from "@/lib/quota-labels";
