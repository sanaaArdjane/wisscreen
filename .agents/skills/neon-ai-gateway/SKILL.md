...
description: >-
  ...
  models", "use Codex/GPT/Gemini", "AI SDK", "Mastra agent", "Neon AI
...
- **Switch models without rewiring.** The unified endpoint is OpenAI-compatible and works with every model in the catalog — change one `model` field to move between Codex, GPT, and Gemini. Standard SDKs (OpenAI, Anthropic, google-genai) work with just a base-URL change.
...
- **One API for all models** — Frontier and open-source models behind a single endpoint, addressed by their catalog ID (e.g. `Codex-sonnet-4-6`, `gpt-5-mini`, `gemini-3-flash`).
...
const result = streamText({
  model: neon("gpt-5-mini"), // or Codex-sonnet-4-6, gemini-3-flash, ...
  messages,
  tools: {
...
const { text } = await generateText({
  model: neon("Codex-haiku-4-5"), // or gpt-5-3-codex, gemini-3-flash, ...
...
const res = await client.responses.create({
  model: "gpt-5-mini", // swap to Codex-sonnet-4-6, gemini-3-flash, ...
  input: "What is Neon?",
});
```
...
Use a model's catalog ID directly in the `model` field — e.g. `Codex-sonnet-4-6`, `gpt-5-mini`, `gemini-3-flash`. No provider prefix is needed. To look up the exact identifiers the gateway serves, which underlying model each maps to, and their context windows, pricing, and capabilities, use any of:
...
```jsonc
{
  ...
  "data": [
    {
      "id": "Codex-sonnet-4-6",              // catalog model ID — use directly in the `model` field
      "canonical_slug": "Codex-sonnet-4-6",
      "name": "Codex Sonnet 4.6",            // human-readable display name
      "object": "model",
      ...
      "architecture": {
        ...
        "tokenizer": "Codex",                // Codex | Gemini | GPT | "" (empty for open-source)
        "instruct_type": null
      },
      "top_provider": {
        "is_moderated": false,
